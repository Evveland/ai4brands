...
const orgTypes = ["Aceleradora","Incubadora","Hub de innovación","Universidad / Facultad","Cámara de comercio","Red de startups","Corporación / Venture","Otro"];

function CreateInstitutionalForm({ userId, onCreated }: { userId: string; onCreated: () => void }) {
  const [name, setName] = useState("");
  const [ecosystemTag, setEcosystemTag] = useState("Aceleradora");
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    await createOrg(userId, { type: "institutional", name, ecosystem_tag: ecosystemTag, contact_email: email, website, description, country });
    setLoading(false);
    onCreated();
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-3">
      <div><label className={labelCls}>Nombre de la organización *</label>
        <input required value={name} onChange={e => setName(e.target.value)} placeholder="Ej: Madrid-in-Game, IE Ventures…" className={inputCls} /></div>
      <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
        <div><label className={labelCls}>Tipo</label>
          <select value={ecosystemTag} onChange={e => setEcosystemTag(e.target.value)} className={inputCls}>
            {orgTypes.map(t => <option key={t}>{t}</option>)}
          </select></div>
        <div><label className={labelCls}>Email</label>
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="hola@ecosistema.com" className={inputCls} /></div>
      </div>
      <div><label className={labelCls}>Web</label>
        <input type="url" value={website} onChange={e => setWebsite(e.target.value)} placeholder="https://ecosistema.com" className={inputCls} /></div>
      <div><label className={labelCls}>Descripción del ecosistema</label>
        <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
          placeholder="Número de startups, sectores, comunidad…" className={`${inputCls} resize-y`} /></div>
...
const steps = [
  { icon: "📣", title: "Invita tu cohort", desc: "Comparte el código con tus startups para que se registren.", xp: "+300 XP", screen: "quests" as const },
  { icon: "🏆", title: "Compite por Ecosystem Champion", desc: "Ranking por calidad de activación.", xp: "+500 XP", screen: "rankings" as const },
  { icon: "🤝", title: "Conecta con agencias y marcas", desc: "Presenta tus mejores startups al mercado.", xp: "+400 XP", screen: "scout" as const },
];

export function InstitutionalQuestPage() {
  const { go } = useNav();
  const { org, loading, view, setView, reload, dbUser } = useOrgQuest();
  const approved = org?.status === "approved";

  if (loading) return <div><BackBar title="Institutional Quest" subtitle="Cargando…" /><div className="grid gap-3">{[1,2,3].map(i => <div key={i} className="rounded-[20px] h-[70px] animate-pulse" style={{ background: "rgba(255,255,255,.06)" }} />)}</div></div>;
  if (view === "no-org") return <div><BackBar title="Institutional Quest" subtitle="Ruta de acceso para ecosistemas" /><NoOrgEntry roleName="Institucional" userId={dbUser?.id ?? ""} onCreateClick={() => setView("create")} onJoined={reload} /></div>;
  if (view === "create") return <div><BackBar title="Registrar ecosistema" subtitle="Tu solicitud será revisada por AI4Brands" /><CreateInstitutionalForm userId={dbUser?.id ?? ""} onCreated={() => { setView("quest"); reload(); }} /></div>;

  return (
    <div>
      <BackBar title={org?.name ?? "Institutional Quest"} subtitle={`${org?.ecosystem_tag ?? "Ecosistema"} · ${org?.role_in_org === "owner" ? "Responsable" : "Miembro"}`} />
      ...
      <div className="rounded-[18px] border p-4 mb-4" style={{ background: "rgba(23,29,52,.85)", border: "1px solid rgba(255,255,255,.08)" }}>
        <div className="text-[13px] font-black text-white mb-2">Perfil del ecosistema</div>
        {[
          { label: "Nombre", value: org?.name },
          { label: "Tipo", value: org?.ecosystem_tag },
